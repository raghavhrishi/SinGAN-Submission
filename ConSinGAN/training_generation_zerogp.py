...
def train_single_scale(netD, netG, reals, fixed_noise, noise_amp, opt, depth, writer):
    ...
    #Print
    print("l",len(netG.body[-opt.train_depth:]))
    print_list = [(len(netG.body[-opt.train_depth:])-1-idx)
               for idx, block in enumerate(netG.body[-opt.train_depth:])]
    #pdb.set_trace()
    print(print_list)
    print("Length of training depth",len(netG.body[-opt.train_depth:]))
    ...
    # optimizerG = optim.Adam(parameter_list, lr=0.01, betas=(0.1, 0.999))
    ...
    if depth == 0:
        ...
        RMSE = 0.5*torch.sqrt(rec_loss).detach()
    ...
    for iter in _iter:
        ...
        for j in range(opt.Dsteps):
            ...
            output = netD(real)
            output_gp = output
            ones = torch.ones_like(output_gp).cuda()
            errD_real = F.binary_cross_entropy_with_logits(output_gp, ones, reduction='none').mean()
            ...
            output_fgp = output
            zeros = torch.zeros_like(output_fgp).cuda()
            errD_fake = F.binary_cross_entropy_with_logits(output_fgp, zeros, reduction='none').mean()

            # ZeroGP
            gradient_penalty = functions.calc_gradient_penalty(netD, real, fake, opt.lambda_grad, opt.device)
            errD_total = errD_real + errD_fake + 10*gradient_penalty
            errD_total.backward()
            optimizerD.step()
        ...
        ###########################
        output = netD(fake)
        ones = torch.ones_like(output).cuda()
        errG = F.binary_cross_entropy_with_logits(output, ones, reduction='none').mean()
        #errG = -output.mean()
        if alpha != 0:
            loss = nn.MSELoss()
